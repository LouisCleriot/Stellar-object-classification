...
def main(input_filepath, output_filepath):
    ...
    NOTE: For more details about the preprocessing of the data, please
    refer to the notebook in notebooks/DataVisualization.ipynb
    """
    ...
    data.drop((data[(data["class"] == "GALAXY")
              & (data["redshift"] == 0)].index))
    ...
    spinner.start()
    # Split the data for training and testing with stratify sampling to keep
    # the same proportion of classes in train and test
    train, test = train_test_split(
        data, test_size=0.2, random_state=0, stratify=data["class"]
    )
